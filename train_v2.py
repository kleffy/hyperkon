...
if __name__ == "__main__":
    ...
    writer_name_tag = f'{experiment_name}_C{channels}_b{batch_size}_e{num_epochs}_OF{out_features}_{tag}'
    writer = SummaryWriter(os.path.join(log_dir, writer_name_tag))
    
    if config.get("resnext") == 101:
        print("Initialised ResNext101!")
        model = resnext101(in_channels=channels, out_features=out_features).to(device)
    elif config.get("resnext") == 152:
        print("Initialised ResNext152!")
        model = resnext152(in_channels=channels, out_features=out_features).to(device)
    elif config.get("resnext") == 50:
        print("Initialised ResNext50!")
        model = resnext50(in_channels=channels, out_features=out_features).to(device)
    ...
    model_name = f'{experiment_name}_C{channels}_b{batch_size}_e{num_epochs}_OF{out_features}_{tag}'
